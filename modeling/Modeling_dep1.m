...
load '../datas/dep1_meanfc.mat';
load '../datas/dep1_meansc.mat';

%% -------------------- Preprocess SC & FC --------------------
SC = dep1_meansc(1:90,1:90);
SC = SC([1:2:90,90:-2:2],[1:2:90,90:-2:2]);
SC = (SC-min(SC(:))) / (max(SC(:))-min(SC(:)));

FC = dep1_meanfc(1:90,1:90);
...
while iT <= 1000
    ...
    for ii = 1 : length(G_coupling)
        ...
        errorlifetime=sqrt(sum((LT2emp-LTime).^2)/length(LTime));
        tmp=0.5*(sum(Pstates.*log(Pstates./P2emp))+sum(P2emp.*log(P2emp./Pstates)));
        tmp(isnan(tmp))=0; kldist=tmp;
        entropydist=EntropyMarkov2(PTR2emp,PTRsim,P2emp,Pstates);
    ...
    if Tolerance == 1
        save('../datas/dep1/WC_G_Coef(initial)','G_Coef');
    end

    if Tolerance==1
        save('../datas/dep1/G_couplingT1','G_coupling');
        save('../datas/dep1/r_ccWC','r_cc');
        save('../datas/dep1/kldistT1','kldists');
    end
    ...
    %% -------------------- SC Update --------------------
    SC2 = SC1;
    load ../datas/signal_dep1_90p.mat
    edep1_bold = signal_dep1_90p;
    for i=1:size(edep1_bold,3)
        singlePCe(:,:,i)=calculatePC(edep1_bold(:,:,i));
        singlePRe(:,:,i)=calculatePR(edep1_bold(:,:,i));
...
save('../datas/dep1/data_need_cluster_finalDMF_testIteration_cb1atanh_allTrial','data_need_cluster')
