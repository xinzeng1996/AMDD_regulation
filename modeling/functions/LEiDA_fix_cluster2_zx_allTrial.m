function [PTRANSITION,Pstates,LTime] = LEiDA_fix_cluster2_zx_gai(BOLDsig,NumClusters,Center,TR)

%BOLDsig = normalize(BOLDsig,'zscore');
[N_areas, Tmax, ntrial]=size(BOLDsig);

bold_tmp = BOLDsig(:,:,1);
for ii=2:ntrial
    bold_tmp = cat(2,bold_tmp,BOLDsig(:,:,ii));
end
BOLDsig = bold_tmp;

% Preallocate variables to save FC patterns and associated information
Leading_Eig=zeros(ntrial*Tmax,1*N_areas); % All leading eigenvectors
...
Phase_BOLD=zeros(N_areas,ntrial*Tmax);

% Get the BOLD phase using the Hilbert transform
for seed=1:N_areas
    %BOLDsig(seed,:)=BOLDsig(seed,:)-mean(BOLDsig(seed,:));
    signal_filt =filtfilt(bfilt,afilt,demean(detrend(BOLDsig(seed,:))));
...
Pstates(isnan(Pstates))=1e-6; Pstates(Pstates==0)=1e-6;
